...
try {
    ...
    $assetCode = strtolower(trim((string)($input['asset_code'] ?? '')));
    $stmt = $pdo->prepare("SELECT * FROM deposit_asset_catalog WHERE canonical_code=? AND provider_enabled=1 AND deposits_enabled=1 LIMIT 1");
    $stmt->execute([$assetCode]);
    $assetConfig = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$assetConfig) {
        json_error('Unsupported deposit asset or network.', 422);
    }

    $currency = strtoupper($assetConfig['symbol']);
    $network = $assetConfig['network_key'];
    $providerNetwork = $assetConfig['provider_network'];
    ...
    $orderId = 'USER_' . $user['id'] . '_' . strtoupper($assetCode) . '_' . time();
    ...
    $payload = [
        ...
        'order_id'         => $orderId,
        'description'      => 'Permanent ' . $currency . ' deposit address for User ID: ' . $user['id'] . ' on ' . $assetConfig['network_name']
    ];
    ...
    try {
        $stmt = $pdo->prepare("
            ...
            (user_id, network, currency, asset, provider, provider_network, address, memo, track_id, provider_track_id, address_hash, status, assignment_status, created_at, last_used_at)
            VALUES (?, ?, ?, ?, 'oxapay', ?, ?, ?, ?, ?, ?, 'active', 'active', NOW(), NOW())
        ");
        $stmt->execute([$user['id'], $network, $currency, $currency, $providerNetwork, $address, $memo, $trackId, $trackId, $addressHash]);
