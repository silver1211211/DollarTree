...
.sw-modal {
  ...
  box-shadow: 0 24px 60px rgba(0,0,0,.48), inset 0 1px 0 rgba(255,255,255,.05);
...
<div class="sw-overlay" id="sw-sup-overlay" onclick="if(event.target===this)closeSupport()">
  <div class="sw-sup-modal">
    ...
    <div class="sw-sup-body">
      ...
      <a href="https://t.me/Dollar_Tree_Official" target="_blank" rel="noopener" class="sw-sup-channel">
...
(function () {
  ...
  function swDraw(angle) {
    ...
    var pixelSize = Math.round(W * dpr);
    if (c.width !== pixelSize || c.height !== pixelSize) {
      c.width = pixelSize;
      c.height = pixelSize;
    }
    ...
    for (var i = 0; i < N; i++) {
      ...
      ctx.font = '800 ' + (lbl.length > 5 ? '11px' : '13px') + ' "Barlow Condensed",Barlow,sans-serif';
      ctx.textBaseline = 'middle';
      ctx.shadowColor = 'rgba(0,0,0,.72)'; ctx.shadowBlur = 2; ctx.shadowOffsetY = 1;
